...
void settings_dump()
{
    ...
    printPgmString(PSTR
		   (" (max instant cornering speed change in delta mm/min)"));
    printPgmString(PSTR
...
void write_settings()
{
    ...
    memcpy_to_eeprom_with_checksum(1, (char *) &settings,
				   sizeof(settings_t));
}
...
int read_settings()
{
	...
	if (!
	    (memcpy_from_eeprom_with_checksum
	     ((char *) &settings, 1, sizeof(settings_t)))) {
	    return (FALSE);
	}
    } else if (version == 1) {
	...
	if (!
	    (memcpy_from_eeprom_with_checksum
	     ((char *) &settings, 1, sizeof(settings_v1_t)))) {
	    return (FALSE);
	}
	settings.acceleration = DEFAULT_ACCELERATION;
	settings.max_jerk = DEFAULT_MAX_JERK;
