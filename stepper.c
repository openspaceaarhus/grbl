...
SIGNAL(TIMER1_COMPA_vect)
{
    ...
    STEPPING_PORT =
	(STEPPING_PORT & ~DIRECTION_MASK) | (out_bits & DIRECTION_MASK);
    ...
    TCNT2 =
	-(((settings.pulse_microseconds - 2) * TICKS_PER_MICROSECOND) / 8);
    TIFR2 |= (1 << TOV2);
...
SIGNAL(TIMER2_OVF_vect)
{
    ...
    STEPPING_PORT =
	(STEPPING_PORT & ~STEP_MASK) | (settings.invert_mask & STEP_MASK);
}
...
void st_init()
{
    ...
    STEPPING_PORT =
	(STEPPING_PORT & ~STEPPING_MASK) | settings.invert_mask;
...
void set_step_events_per_minute(uint32_t steps_per_minute)
{
    ...
    cycles_per_step_event =
	config_step_timer((TICKS_PER_MICROSECOND * 1000000 * 60) /
			  steps_per_minute);
}
